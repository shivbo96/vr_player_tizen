...
class VrPlayerController {
  ...
  /// Enable/disable 360/VR mode explicitly
  ...
  Future<void> simulateTouch(int eventType, double x, double y) async {
    await _channel.invokeMethod('simulateTouch', <String, dynamic>{
      'eventType': eventType,
      'x': x,
      'y': y,
    });
  ...
  Future<void> startContinuousDrag(double dx, double dy) async {
    await _channel.invokeMethod('startContinuousDrag', <String, dynamic>{
      'dx': dx,
      'dy': dy,
    });
  }

  Future<void> stopContinuousDrag() async {
    await _channel.invokeMethod('stopContinuousDrag');
  }
